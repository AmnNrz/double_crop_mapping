...
def scale_bands_Landsat457(image):
    ...
    return (
        image.addBands(NIR, None, True)
        .addBands(red, None, True)
        .addBands(blue, None, True)
    )
...
def add_system_start_time_image(image):
    return image.addBands(
        image.metadata("system:time_start").rename("system_start_time")
    )
...
def split_feacturecollection_to_blocks(
    fc, simplify=0, block_size=1000, sel_polygon=True
):
    blocks = dict()
